...
export class AddressService {
  ...
  constructor(
    ...
    @InjectRepository(ContactDetails) private addressRepo: Repository<ContactDetails>,
    @InjectRepository(Country) private countryRepo: Repository<Country>,
    @InjectRepository(LocationInfo)
    private locationInfoRepo: Repository<LocationInfo>
  ) {}

  createContactDetails(request: ContactDetailsRequest, reference: string, referenceType: ReferenceType){
      var createAddressDto = [];
      var emails = request.emailAddress?.map((x) => ({
        ...x,
        contactType : ContactType.Email
      }));

      var phones = request.phoneNumbers?.map((x) => ({
        ...x,
        contactType : ContactType.Phone
      }));

      var other = request.otherAddress?.map((x) => ({
        ...x,
        contactType : ContactType.Other
      }));

      if(emails) createAddressDto.push(...emails);
      if(phones) createAddressDto.push(...phones);
      if(other) createAddressDto.push(...other);

      this.create(createAddressDto, reference, referenceType);
      this.createLocation(request.location, reference, referenceType);
  }

  createLocation(request: LocationModel[], reference : string, referenceType: ReferenceType) {
    var locations = request.map((loc) => {
  ...
  create(request: IndividualAddressDto[],reference : string, referenceType: ReferenceType) {
    var address = request.map((add) => {
      var address = this.addressRepo.create(add);
