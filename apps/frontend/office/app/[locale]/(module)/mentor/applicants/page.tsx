...
import {
    ...
    fetchMentorships,
} from 'app/[locale]/_api/mentors/fetch-mentorships';
...
// import type { MentorshipResponse } from 'app/[locale]/_api/admin/fetch-mentorship';
...
const Page = () => {
    ...
    const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } =
        useInfiniteQuery<MentorshipResponse, Error>({
    ...
    const jobOptions = jobs.map((job) => ({
        value: job.program.id,
        label: job.program.title,
    }));

    useEffect(() => {
        if (jobs.length > 0 && !selectedJob) {
            setSelectedJob(jobs[0] ? jobs[0].program.id : '');
        }
    }, [jobs, selectedJob]);
    ...
    return (
        <Box className="w-full flex flex-col items-center gap-2.5">
            <Paper className="p-4 w-full">
                <h1 className="text-xl font-bold mb-4">Select a Programs</h1>
                <Select
                    className="max-w-md"
                    label="Programs"
