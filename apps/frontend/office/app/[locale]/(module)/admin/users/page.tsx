...
import { Flex, Paper, Table, Text, TextInput, Select, Group, Menu, ActionIcon, Tooltip, Divider, TableScrollContainer, Checkbox } from "@mantine/core";
...
const exportToCSV = (users: Users[]) => {
  const header = ['Full Name', 'Email', 'Role', 'Status', 'Created By', 'Created At'];
...
export const UsersPage = () => {
  ...
  const rows = sortedUsers.map((user) => (
    <Table.Tr key={user.id} style={{ transition: 'background-color 0.3s ease', cursor: 'pointer' }}>
        <Table.Td>
    ...
    <Paper shadow="xs" p="lg" style={{ borderRadius: '10px' }}>
      ...
      <Group justify="space-between" className="mb-4">
        ...
        <Group>
          ...
          <Tooltip label="Export to CSV" withArrow>
            <ActionIcon onClick={() => exportToCSV(sampleUsers)} variant="outline" color="green" radius="xl" size="sm">
      ...
      <TableScrollContainer minWidth={800} type="native">
        <Table withRowBorders withColumnBorders  striped verticalSpacing={"md"}>
          <Table.Thead style={{ backgroundColor: '#f4f4f4', fontWeight: 'bold' }}>
            <Table.Tr>
            <Table.Th>
                <Checkbox
                onChange={toggleAll}
