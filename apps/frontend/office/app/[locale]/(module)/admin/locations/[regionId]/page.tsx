...
import { fetchCities, addRegion } from "app/[locale]/_api/job-details";
      ...
      {cities.length === 0 ? (
        ...
        <>
          <Card mt="md" padding="md" shadow="sm" mb={"md"}>
            <Text className="font-bold text-lg">{t("selectedRegion")}</Text>
            <Text>
              ...
              <Table.Tbody>
                {cities.map((city) => (
                  <Table.Tr key={city.id}>
                    ...
                    <Table.Td>{city.isActive ? "Yes" : "No"}</Table.Td>
